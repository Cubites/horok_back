...
public interface FavorsRepository extends JpaRepository<Favors,Integer> {
    ...
    @Query("SELECT f.folderReview.folderReviewId, COUNT(f) FROM Favors f WHERE f.folderReview.folderId = :folderId GROUP BY f.folderReview.folderReviewId ORDER BY f.folderReview.folderReviewId ")
