...
public class JPAEntityTest {
    ...
    public void userFolderJoinAndCreateTest() {
        ...
        // 폴더 생성
        // 생성할 폴더
        Folders folder1 = Folders.builder().folderName("f2").folderImg("path").participantsList(new ArrayList<>())
                .build();
        // 참가할 폴더
        Folders folders2 = Folders.builder().folderName("f1").folderImg("path").participantsList(new ArrayList<>())
                .build();
        // 이미 생성된 폴더
        ...
        assertThat(findFolder.getParticipantsList().get(0).getFolderParticipantsId())
                .isEqualTo(pa1.getFolderParticipantsId());
    ...
    // //폴더 생성
    // //리뷰남길 폴더
    // Folders folder1 = Folders.builder().folderName("뱁").folderImg("path").
    // participantsList(new ArrayList<>()).folderList(new ArrayList<>()).build();
    // //리뷰안남길 폴더
    // Folders folders2 =
    // Folders.builder().folderName("김").folderImg("path").participantsList(new
    // ArrayList<>()).folderList(new ArrayList<>()).build();
    //
    // //리뷰 남길 유저 -> 이미 등록된 유저이다.
    // Users findUser = userRepository.findByUserNickname("강태바리");
    //
    // //먼저 유저가 폴더에 참가
    // Participants paticipants1 =
    // Participants.builder().folderFavor(false).build();
